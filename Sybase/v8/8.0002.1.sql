if not exists(select 1 from systable where table_name = 't_transasscas') then 
				CREATE TABLE t_transasscas (  id_transasscas INTEGER NOT NULL DEFAULT autoincrement, 
																						id_visit INTEGER NULL, 
																						compte VARCHAR(3) NULL, 
																						datetrans DATE NULL, 
																						montant DECIMAL(8,2) NULL, 
																						prixlab DECIMAL(8,2) NULL, 
																						prixlab2 DECIMAL(8,2) NULL, 
																						PRIMARY KEY(id_transasscas) 
									) 
				end if;

if (select count(1) from t_transasscas) = 0 then 
